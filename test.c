...
main(int argc, char **argv, char **envp){
        ...
        while (1) {
                ...
                outlonghex(-38, u_in.rax);      /* seems constant */
                fprintf(stderr, " %08lx %08lx %08lx",
                        u_in.rbx, u_in.rcx, u_in.rdx);
                ...
                if (syscall == __NR_execve) {
                         ...
                         */
                        eax = my_ptrace_read(PTRACE_PEEKUSER, p, &regs[RAX],0);
                        if (eax == 0) {
                ...
                outlonghex(u_in.rax, u_out.rax);
                outlonghex(u_in.rbx, u_out.rbx);
                outlonghex(u_in.rcx, u_out.rcx);
                outlonghex(u_in.rdx, u_out.rdx);
                fprintf(stderr, "\n");
