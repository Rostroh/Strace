...
void		print_sigcode_chld(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGCHLD {si_signo=SIGCHLD, si_code=%s, si_pid=%d, si_uid=%d, si_status=%d, si_utime=%d, si_stime=%d} ---\n", sys_code[code - 1], info.si_pid, info.si_uid, info.si_status, info.si_utime, info.si_stime);
}
...
void		print_sigcode_int(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGINT {si_signo=SIGINT, si_code=");
	for (int i = 0; i < 8; i++)
	{
		if (sys_val[i] == code)
		{
...
void		print_sigcode_segv(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGSEGV {si_signo=SIGSEGV, si_code=");
	for (int i = 0; i < 2; i++)
	{
		if (sys_val[i] == code)
		{
	...
	fprintf(stderr, "[1]\t%d segmentation fault (core dumped)\n", getpid());
	exit(0);
}
...
void		print_sigcode_fpe(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGFPE {si_signo=SIGFPE, si_code=");
	for (int i = 0; i < 2; i++)
	{
		if (sys_val[i] == code)
		{
	...
	fprintf(stderr, ", si_addr=%p} ---\n", info.si_addr);
	fprintf(stderr, "+++ killed by SIGFPE (core dumped) +++\n");
	fprintf(stderr, "[1]\t%d Floating point exception (core dumped)\n", getpid());
	exit(0);
}
...
void		print_sigcode_bus(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGBUS {si_signo=SIGBUS, si_code=");
	for (int i = 0; i < 2; i++)
	{
		if (sys_val[i] == code)
		{
	...
	fprintf(stderr, ", si_addr=%p} ---\n", info.si_addr);
	fprintf(stderr, "+++ killed by SIGBUS (core dumped) +++\n");
	fprintf(stderr, "[1]\t%d bus error (core dumped)\n", getpid());
	exit(0);
}
...
void		print_sigcode_ill(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGILL {si_signo=SIGILL, si_code=");
	for (int i = 0; i < 2; i++)
	{
		if (sys_val[i] == code)
		{
	...
	fprintf(stderr, ", si_addr=%p} ---\n", info.si_addr);
	fprintf(stderr, "+++ killed by SIGILL (core dumped) +++\n");
	fprintf(stderr, "[1]\t%d illegal instruction (core dumped)\n", getpid());
	exit(0);
}
...
void		print_sigcode_poll(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGPOLL {si_signo=SIGPOLL, si_code=");
	for (int i = 0; i < 6; i++)
	{
		if (sys_val[i] == code)
		{
	...
	fprintf(stderr, ", si_band=%ld, si_fd=%d} ---\n", info.si_band, info.si_fd);
}
...
void		print_sigcode_winch(siginfo_t info, int code)
{
	...
	fprintf(stderr, "--- SIGWINCH {si_signo=SIGWINCH, si_code=");
	for (int i = 0; i < 8; i++)
	{
		if (sys_val[i] == code)
		{
